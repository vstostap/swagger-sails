...
module.exports = function swaggerHook(sails) {
  ...
  return {
    ...
    initialize: function initialize(cb) {
      ...
      if(sails.config.swagger && !sails.config.swagger.active) {
          return cb();
      }
    ...
    routes: {
      after: {
        [sails.config.blueprints.prefix + '/swagger']: function(req, res, next) {
            ...
            if (!operation) {
                ...
                operation = {pathObject: { 'x-swagger-pipe': 'swagger_raw' }};
            }
